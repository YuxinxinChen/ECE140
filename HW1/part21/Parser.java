...
public class Parser {
    ...
    TK f_statement[] = {TK.ID, TK.PRINT, TK.IF, TK.DO, TK.FA, TK.SKIP, TK.STOP, TK.BREAK, TK.DUMP, TK.EXCNT, TK.none};
    ...
    private int[] excntPool;
    private int currentExcnt = 0;
    private void scan() {
        tok = scanner.scan();
    }
	...
	this.excntPool = new int[100];
	Arrays.fill(excntPool, -1);
        scan();
        program();
        if( tok.kind != TK.EOF )
	...
	gcprint("int EXCNT[100]={0};");
	gcprint("int EXCNT_Line[100]={-1};");
	...
	gcprint("void printEXCNT(int max){printf(\"---- Execution Counts ----\\n num line    count\\n\"); for(int i=0; i<max; i++){printf(\"%4d%5d%9d\\n\",i+1,EXCNT_Line[i],EXCNT[i]);}}");

        gcprint("int main() {");
	block();
	if(currentExcnt>0) {
	    gcprint("STOPJUMP:");
	    for(int i=0; i<currentExcnt; i++) {
		    gcprint("EXCNT_Line["+i+"]="+excntPool[i]+";");
	    }
	    gcprint("printEXCNT("+currentExcnt+");");
	}
        gcprint("return 0; }");
    }
    ...
    private void excnt(){
	int excnt_linenumber = tok.lineNumber;
	mustbe(TK.EXCNT);
	if(currentExcnt >= 100) {
		System.err.println("can't parse: line "+excnt_linenumber+" too many EXCNT (more than 100)");
		System.exit(1);
	}
	int index = -2;
	index = Arrays.asList(excntPool).indexOf(excnt_linenumber );
	if(index != -1) gcprint("EXCNT["+index+"]++;");
        else if(index == -1) {
		excntPool[currentExcnt] = excnt_linenumber;
		gcprint("EXCNT["+currentExcnt+"]++;");
		currentExcnt++;
	}	
	else if(index <= -2 || index >= 100) System.err.println("weird thing happened");
    }
