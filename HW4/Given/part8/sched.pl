...
naaa(L,NAL,AL):-
        ...
        (L=[H|T],
        naaa(T,R1,R2),
        ((\+(atom(H))) -> (NAL=[H|R1], AL=R2);
         atom(H) -> (NAL=R1, AL=[H|R2]))).
...
fitanyRequest([Owner,Size], MemList, NewMemList):-
        ...
        (MemList=[H|T],
         H=[A,S,O],
        ((S>=Size, O==z) -> (splitlistRight(T, N1), Newadd is A+Size, Newsize is S-Size, X1=[A,Size,Owner], 
                                (Newsize==0 -> NewMemList=[X1|N1];
                                NewMemList=[X1,[Newadd, Newsize, O]|N1])))).

fitanyRequest([Owner,Size], MemList, NewMemList):-
        MemList=[H|T],
        fitanyRequest([Owner, Size], T, N), 
        NewMemList=[H|N].

fit1st(RRList, MemList, NewMemList):-
        RRList=[]->NewMemList=MemList;
        (RRList=[H|T],
        (is_list(H) -> fit1stRequest(H, MemList, N); 
         fitRelease(H, MemList, N)
        ),  
        fit1st(T,N, NewMemList)).

fitany(RRList, MemList, NewMemList):-
        RRList=[]->NewMemList=MemList;
        (RRList=[H|T],
        (is_list(H) -> fitanyRequest(H, MemList, N);
         fitRelease(H, MemList, N)
        ),
        fitany(T,N, NewMemList)).

fit1stTryHarder(RRList, MemList, NewRRList, NewMemList):-
        fit1st(RRList, MemList, NewMemList) -> fail;
        (permsub(RRList, NewRRList),
         (NewRRList==RRList -> fail; 
